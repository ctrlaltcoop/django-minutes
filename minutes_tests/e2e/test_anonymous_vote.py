...
from minutes_tests.scenarios.anonymous_vote_scenario import AnonymousVoteScenario


class AnonymousVoteTest(LiveServerTestCase):
    def setUp(self) -> None:
        self.client = APIClient()
        self.scenario = AnonymousVoteScenario()

    def test_401_for_unauthenticated_user_on_list(self):
        response = self.client.get('/api/v1/anonymousvote/')
        self.assertEqual(response.status_code, 401)

    def test_401_for_unauthenticated_user_on_retrieve(self):
        response = self.client.get('/api/v1/anonymousvote/{0}/'.format(self.scenario.anonymous_vote.id))
        self.assertEqual(response.status_code, 401)
    ...
    def test_200_for_owner_on_list_containing_my_meeting(self):
        ...
        self.assertIn(self.scenario.anonymous_vote.id, vote_ids)
        self.assertNotIn(self.scenario.another_anonymous_vote.id, vote_ids)
    ...
    def test_200_for_filtering_by_meeting_and_contains_only_requested(self):
        ...
        response = self.client.get('/api/v1/anonymousvote/', {'decision': self.scenario.decision.id})
        vote_ids = [i['id'] for i in response.json()]
        self.assertEqual(response.status_code, 200)
        self.assertIn(self.scenario.anonymous_vote.id, vote_ids)
        self.assertNotIn(self.scenario.another_anonymous_vote.id, vote_ids)
    ...
    def test_200_for_filtering_by_meeting_2_and_contains_only_requested(self):
        ...
        response = self.client.get(
            '/api/v1/anonymousvote/',
            {'decision': self.scenario.meeting_2_agenda_item_decision.id}
        )
        vote_ids = [i['id'] for i in response.json()]
        self.assertEqual(response.status_code, 200)
        self.assertNotIn(self.scenario.anonymous_vote.id, vote_ids)
        self.assertIn(self.scenario.meeting_2_vote.id, vote_ids)
    ...
    def test_404_for_nobody_on_delete(self):
        self.client.credentials(HTTP_AUTHORIZATION='Token ' + self.scenario.nobody_token.key)
        response = self.client.delete('/api/v1/anonymousvote/{0}/'.format(self.scenario.anonymous_vote.id))
        self.assertEqual(response.status_code, 404)
    ...
    def test_400_for_participant_on_retrieve_after_delete(self):
        ...
        response = self.client.get('/api/v1/anonymousvote/{0}/'.format(self.scenario.anonymous_vote.id))
        self.assertEqual(response.status_code, 200)
        self.scenario.anonymous_vote.delete()
        response = self.client.get('/api/v1/anonymousvote/{0}/'.format(self.scenario.anonymous_vote.id))
        self.assertEqual(response.status_code, 404)
